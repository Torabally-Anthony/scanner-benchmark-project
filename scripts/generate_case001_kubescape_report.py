...
MATCHED_FILE = Path("results/matched/kubescape/case-001-privileged-container.matched.json")
METRICS_FILE = Path("results/metrics/kubescape/case-001-privileged-container.metrics.json")
REPORT_FILE = Path("results/reports/case-001-kubescape-report.md")
...
def main() -> None:
    ...
    report_lines.append("# Case 001 Kubescape Benchmark Report")
    report_lines.append("")
    ...
    report_lines.append(f"- Normalised Kubescape findings: `{summary['normalised_findings_count']}`")
    ...
    if false_negatives:
        ...
        report_lines.append("No false negatives were found. Kubescape detected the labelled ground-truth issue.")
        report_lines.append("")
    ...
    for item in unlabelled_extras:
        report_lines.append(
            f"- `{item['scanner_rule_id']}` — {item['scanner_rule_name']} ({item['severity']})"
        )
    ...
    report_lines.append("")
    report_lines.append(
        "Kubescape successfully detected the privileged container misconfiguration using control `C-0057`. "
        "This finding matched the ground truth label `GT-001`, so it was classified as a true positive."
    )
    report_lines.append("")
    report_lines.append(
        "The additional Kubescape failed controls were stored as unlabelled extras. "
        "They may represent valid security recommendations, but they are not part of the current ground-truth label set for this controlled benchmark case."
    )
    report_lines.append("")
    ...
    report_lines.append("")
    report_lines.append(
        "The first Kubescape benchmark run was successful. For case-001, Kubescape detected the intended privileged container issue, "
    ...
    print("Kubescape report generated.")
